    ...
    (() =>
    {
        unsafeWindow.LDM =
        {
            ...
            process_page: (id, pg = 1, cb = null, pcr = null) =>
            {
                ...
                $.ajax({
                    ...
                    error: (UCWinb, status, info) =>
                    {
                        ...
                        setTimeout(() => LDM.process_page(id, pg, cb, pcr), 100);
                    }
                });
                return;
            },
            process_discuss: (id, delay, cb = null) =>
            {
                if (!is.num(id)) throw `postid 传错了，应该是数字，憨憨！拿着你的 ${typeof id} 爪巴！`;
                let get_page_num = (p, f) =>
                {
                    ...
                    f.away();
                }, pg = parseInt((data.read_reply_num[id] ? data.read_reply_num[id] : 0) / 10), weight = [], marked_user = [], n = Infinity;
                let f = i => LDM.process_page(id, pg + i, () =>
                {
                    if (i <= n) setTimeout(() => f(i + 1), delay);
                    else cb(weight);
                },
                [
                    i ? ef : get_page_num,
        ...
        f = i =>
        {
            ...
            if (LDM.compare_time(time, data.dead_time))
            {
                ...
                if (is.empty(data.cool_down[id]) || new Date().valueOf() - data.cool_down[id] > 180000)
                    // Info: 请勿擅自修改此值以免对洛谷（服务器）造成不必要的负担。（三分钟才能分析一次）
                {
                    data.cool_down[id] = new Date().valueOf();
                    LDM.process_discuss(id, 2000, weight => // Info: 这个 2000 最好不要改，是延迟。
                    {
                        $r.html("");
                        data.mark_record[id] = [];
                        for (let j in weight)
                        ...
                        if (data.mark_record[id].length === 0)
                        LDM.add_badge($r, "bluelight", "nothing", "暂无标记");
                    });
                }
            ...
            if (i < $d.length - 1) setTimeout(() => f(i + 1), 200);
